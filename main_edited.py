...
class Labeling:
    ...
    def __init__(self,imgfile, model) -> None:
        ...
        self.SKEW_CORRECTION = skew_correction(image3)
        self.DOCTR        = get_doctr_labels(model, self.imgfile, image)
        self.EASYOCR      = get_EasyOCR_BB(image3)
...
@preprocessor()
def get_easyocr_info(x):
    return lf.EASYOCR[x[0]][x[1]]

@preprocessor()
def get_correct_skew(x):
    return lf.SKEW_CORRECTION[x[0]][x[1]]
...
@labeling_function(label=pixelLabels.TEXT, pre=[get_easyocr_info], name="EASYOCR")
def EASYOCR_LABEL(pixel):
    if(pixel):
        return pixelLabels.NOT_TEXT
    else:
        return ABSTAIN

@labeling_function(label=pixelLabels.TEXT, pre=[get_correct_skew], name="SKEW")
def SKEWCORRECTION_LABEL(pixel):
    if(pixel):
        return pixelLabels.NOT_TEXT
    else:
        return ABSTAIN
...
def main(img):
    ...
    LFS = [ 
        ...
        SKEWCORRECTION_LABEL, EASYOCR_LABEL
...
def cage(img, X, Y):
    ...
    LFS = [ 
        ...
        DOCTR_LABEL, TESSERACT_LABEL, CONTOUR_LABEL,
        # MASK_HOLES_LABEL#, MASK_OBJECTS_LABEL
        #EASYOCR_LABEL, SKEWCORRECTION_LABEL
    ]
